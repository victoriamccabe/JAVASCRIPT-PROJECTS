...
//This functoin adds the inputed task to the get_todos function array
function add() {
    //This takes the inputed task and creates a variable of it
    var task = document.getElementById('task').value;

    var todos = get_todos();
    //tjos addsa a new task the end of the array
    todos.push(task);
...
function show() {
    ...
    var html = '<ul>';
    //this displays a task to the list in th eorder that it is inputed
    for(var i = 0; i < todos.length; i++) {
        //this also displays the task as a list and creates the button with the "x"
        html += '<li ondblclick="myFunction(this)">' + todos [i] + '<button class="remove" id="' + i + '">x</button></li>';
    };
    html += '</ul>';
...
// Function to change the color of the <li> element to red on double-click
